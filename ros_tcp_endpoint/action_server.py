...
"""
Bridge a Godot-implemented Action server through ROS-TCP-Endpoint.

When a ROS 2 action client sends a goal, the execute_callback
forwards it to Godot using the same __request/__response two-frame
pair that regular unity_service uses. Godot computes and sends the
result back; feedback is pushed from Godot via a separate
...
class RosActionServer(RosSender):
    ...
    def _execute_callback(self, goal_handle):
        """Forward the goal to Godot and wait for the result."""
        ...
        # Send the goal to Godot using the __request / __response
        # mechanism. send_unity_service_request blocks until Godot
        # responds. We use the action_name as the topic/destination
        # so Godot's ROSActionServer can route it.
        #
        ...
        result_cdr = self._send_goal_to_godot(goal_uuid, goal_cdr)
    ...
    def _send_goal_to_godot(self, goal_uuid, goal_cdr):
        """Send a goal to Godot and wait for the result CDR bytes.
        ...
        # 16-byte goal UUID + CDR goal body. Godot strips the UUID
        ...
        # Block until Godot sends __response{srv_id} with the result.
    ...
    def publish_feedback(self, goal_uuid_bytes, feedback_cdr):
        """Called when Godot sends feedback for an in-progress goal."""
        with self._lock:
